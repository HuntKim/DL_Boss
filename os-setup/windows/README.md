...
- 계정은 `New-LocalUser` 생성 시 이 값으로 설정된다. **"다음 로그온 시
  반드시 암호 변경" 강제(WinNT ADSI provider의 `PasswordExpired` 속성)는
  걸지 않는다** - hiware 쪽에서 이미 로그온 시 비밀번호 변경을 처리하고
  있어서, OS 레벨에서 중복으로 강제하면 오류가 날 수 있다는 피드백에
  따라 뺐다. 담당자가 이 초기 비밀번호로 로그온해 비밀번호를 바꾸는
  절차는 hiware를 통해 이뤄진다.
...
- **`config/os_env/<hostname>.ps1`은 다른 host env 값들과 마찬가지로 git에
  ...
  노출이다. hiware를 통해 로그온 시 비밀번호가 실제로 바뀐다는 전제(그리고
